...
public class Queen extends Piece{
    private final Integer[] moveVectors = new Integer[]{-9, -8, -7, -1, 1, 7, 8, 9};

    public Queen(Integer position, Color color, Boolean isFirstMove) {
    ...
    private boolean isLegalMove(Integer currentPosition, Integer moveVector){
        return (isValidTile(currentPosition + moveVector) &&
                !(isOnFirstFile[currentPosition] && (moveVector == -1 || moveVector == -9 || moveVector == 7) ||
                    isOnEighthFile[currentPosition] && (moveVector == 1 || moveVector == -7 || moveVector == 9)));
    }
