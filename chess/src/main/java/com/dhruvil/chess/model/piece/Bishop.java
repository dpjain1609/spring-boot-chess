...
public class Bishop extends Piece{
    private final Integer[] moveVectors = new Integer[]{-9, -7, 7, 9};

    public Bishop(Integer position, Color color, Boolean isFirstMove) {
    ...
    private boolean isLegalMove(Integer currentPosition, Integer moveVector){
        return (isValidTile(currentPosition + moveVector) &&
                !(isOnFirstFile[currentPosition] && (moveVector == -9 || moveVector == 7) ||
                isOnEighthFile[currentPosition] && (moveVector == -7 || moveVector == 9)));
    }
